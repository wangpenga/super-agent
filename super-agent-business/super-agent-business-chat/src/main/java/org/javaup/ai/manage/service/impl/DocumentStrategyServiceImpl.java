...
import org.springframework.ai.embedding.EmbeddingModel;        // Spring AI Embedding 模型
...
import java.util.Collections;      // 集合工具（排序/二分查找）
...
public class DocumentStrategyServiceImpl implements DocumentStrategyService {
    ...
    /**
     * Embedding 模型延迟获取器 — 用于 Embedding Cosine 语义切块。
     * 不可用时自动降级到 Jaccard 方案，无感回退。
     */
    ...
    // ═══════════════════════════════════════════════════════════
    //  语义切块执行（applySemanticChunking）— 入口调度
    // ═══════════════════════════════════════════════════════════

    /**
     * 对 ChunkCandidate 列表执行语义切块（默认 CHILD 管线）。
     * <p>
     * 优先使用 Embedding Cosine 方案（需 EmbeddingModel 可用），
     * 不可用时自动降级到 Jaccard 字面相似度方案。
     */
    ...
    /**
     * 对 ChunkCandidate 列表执行语义切块 — 入口调度方法。
     * <p>
     * 调度策略：
     * <ol>
     *   <li>如果 EmbeddingModel 可用 → {@link #applyEmbeddingSemanticChunking}</li>
     *   <li>如果 EmbeddingModel 不可用 → {@link #applyJaccardSemanticChunking}（降级）</li>
     * </ol>
     */
    private List<ChunkCandidate> applySemanticChunking(
        ...
        // 优先使用 Embedding Cosine 语义切块
        EmbeddingModel embeddingModel = embeddingModelProvider.getIfAvailable();
        if (embeddingModel != null) {
            return applyEmbeddingSemanticChunking(sourceList, pipelineType, embeddingModel);
        }
        // EmbeddingModel 不可用时降级到 Jaccard 方案
        return applyJaccardSemanticChunking(sourceList, pipelineType);
    }

    // ═══════════════════════════════════════════════════════════
    //  Embedding Cosine 语义切块（主方案）
    // ═══════════════════════════════════════════════════════════

    /**
     * 对 ChunkCandidate 列表执行 Embedding Cosine 语义切块。
     * <p>
     * 将每个 ChunkCandidate 的文本按句子切分后，用滑窗方式逐窗口
     * 调用 {@link EmbeddingModel#embed} 生成向量，通过相邻窗口的
     * 余弦相似度检测主题边界，相似度低于阈值时触发切分。
     * <p>
     * 阈值支持两种模式：
     * <ul>
     *   <li><b>固定阈值</b>（dynamicPercentile=0）：使用配置的固定 threshold</li>
     *   <li><b>动态阈值</b>（dynamicPercentile>0）：取所有相似度的指定百分位值</li>
     * </ul>
     *
     * @param sourceList    输入的切块候选列表
     * @param pipelineType  管线类型（决定 minChars/maxChars）
     * @param embeddingModel Embedding 模型
     * @return 切分后的候选列表
     */
    private List<ChunkCandidate> applyEmbeddingSemanticChunking(
            List<ChunkCandidate> sourceList,
            DocumentStrategyPipelineTypeEnum pipelineType,
        ...
        for (ChunkCandidate candidate : sourceList) {
            ...
            resultList.addAll(embeddingSemanticSplit(candidate, pipelineType, embeddingModel));
        }
        return resultList;
    }

    /**
     * Embedding Cosine 语义切块核心算法 — 滑窗+余弦相似度检测主题边界。
     * <p>
     * === 算法步骤 ===
     * <ol>
     *   <li>按句号将文本拆分为句子列表</li>
     *   <li>用滑窗（大小=windowSize，步长=1）构建句子窗口</li>
     *   <li>批量调用 embedding 模型生成各窗口的向量</li>
     *   <li>计算相邻窗口的余弦相似度</li>
     *   <li>根据相似度阈值（固定或动态）确定切分点</li>
     *   <li>在切分点处分块，同时受 minChars/maxChars 约束</li>
     * </ol>
     * <p>
     * 句子太少（≤ windowSize）时不切分，整块保留。
     *
     * @param candidate     待切分的 ChunkCandidate
     * @param pipelineType  管线类型
     * @param embeddingModel Embedding 模型
     * @return 切分后的候选列表
     */
    private List<ChunkCandidate> embeddingSemanticSplit(
        ...
        // 句子太少（≤ windowSize）→ 不切，整块保留
        int windowSize = properties.getChunk().getEmbeddingSemanticWindowSize();  // 默认 3
        if (sentenceList.size() <= windowSize) {
            resultList.add(candidate);
            return resultList;
        }
        ...
        // ── Step A: 滑窗构建句子窗口 ──
        // 每个窗口包含 windowSize 个连续句子，步长=1
        // 共得到 sentenceList.size() - windowSize + 1 个窗口
        List<String> windows = new ArrayList<>();
        for (int i = 0; i <= sentenceList.size() - windowSize; i++) {
            // 将窗口内的句子拼接为一个字符串用于 embedding
        ...
        // ── Step D: 确定切分阈值（动态或固定） ──
        double threshold = resolveEmbeddingThreshold(similarities);

        // ── Step E: 按相似度切分句子 ──
        // 遍历每个句子，累积到 currentChunk 中
        // 当遇到主题边界（相似度 < 阈值）且当前块 >= minChars 时切分
        // 当累积长度超过 maxChars 时强制切分
        StringBuilder currentChunk = new StringBuilder();
        for (int i = 0; i < sentenceList.size(); i++) {
            // 累积当前句子
            currentChunk.append(sentenceList.get(i));

            // 边界 1：累积长度 >= maxChars → 强制切分
            boolean exceedMaxChars = currentChunk.length() >= semanticMaxChars;

            // 边界 2：当前块 >= minChars 且该位置检测到主题边界 → 语义切分
            // 窗口 i-windowSize 和 i-windowSize+1 的分界处对应句子 i 和 i-1 之间
            // similarities 的索引 = i - windowSize
            int simIndex = i - windowSize;
            boolean semanticBreak = currentChunk.length() >= semanticMinChars
                && i >= windowSize
                && simIndex < similarities.size()
                && simIndex >= 0
                && similarities.get(simIndex) < threshold;

            if ((exceedMaxChars || semanticBreak) && currentChunk.length() > 0) {
                // 执行切分
    ...
    /**
     * 决议 Embedding Cosine 的切分阈值 — 支持固定和动态两种模式。
     * <p>
     * <ul>
     *   <li>动态模式（dynamicPercentile > 0）：取所有相邻相似度的指定百分位值。
     *       例如 0.15 表示取第 15% 分位，适用于不同文档分布差异大的场景。</li>
     *   <li>固定模式（dynamicPercentile <= 0）：直接返回配置的固定阈值。</li>
     * </ul>
     *
     * @param similarities 所有相邻窗口的余弦相似度列表
     * @return 切分阈值
     */
    private double resolveEmbeddingThreshold(List<Double> similarities) {
        // 读取动态百分位配置（默认 0.15）
        double dynamicPercentile = properties.getChunk().getEmbeddingSemanticDynamicPercentile();
        if (dynamicPercentile <= 0 || similarities.isEmpty()) {
            // 固定阈值模式
            return properties.getChunk().getEmbeddingSemanticThreshold();
        }
        // 动态阈值：对相似度排序，取指定百分位的值
        List<Double> sorted = new ArrayList<>(similarities);
        Collections.sort(sorted);
        int index = (int) Math.ceil(dynamicPercentile * sorted.size()) - 1;
     ...
     * <p>
     * cos(a, b) = a·b / (|a| × |b|)
     * 范围 [-1, 1]，在 embedding 归一化场景下通常为 [0, 1]。
     */
    private double cosineSimilarity(float[] a, float[] b) {
        // 参数校验：任一为空或长度不一致 → 返回 0
        if (a == null || b == null || a.length != b.length || a.length == 0) {
            return 0D;
        }
        double dot = 0D;      // 点积
        double normA = 0D;    // 向量 a 的 L2 范数平方
        double normB = 0D;    // 向量 b 的 L2 范数平方
    ...
    /**
     * 对 ChunkCandidate 列表执行 Jaccard 语义切块 — Embedding 不可用时的降级方案。
     * <p>
     * 用 Jaccard 字面 token 重叠度检测主题边界。
     * 对每个块的文本长度超过 semanticMinChars 才做语义切分（太短的文本无意义），
     * 否则直接保留原块不变。
     */
    private List<ChunkCandidate> applyJaccardSemanticChunking(
            List<ChunkCandidate> sourceList,
            DocumentStrategyPipelineTypeEnum pipelineType) {
