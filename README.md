...
**Super Agent** 是一个 **企业级** 的 AI 智能体对话平台，覆盖了智能对话、文档知识问答、联网搜索、MCP 工具协议、Skills 能力扩展、文档全生命周期管理等完整能力。
...
> [!IMPORTANT]
> **此项目不是 Spring AI / Spring AI Alibaba 的套壳项目。** 框架提供的是模型调用、向量存储这些基础能力，但三层执行器路由、前置编排器五步决策链、双通道检索融合、Parent-Child 块聚合、证据预算控制与无证据短路、组合式切块引擎、摘要压缩记忆、集群级租约互斥、全链路观测追踪——这些构成项目核心竞争力的能力，全部是在框架之上从零设计和手写实现的。光靠框架远远撑不起一个企业级 AI 系统，真正的深度在框架之外。

# 目录

- [为什么需要这个项目](#为什么需要这个项目)
- [学 AI 容易踩的坑](#学-ai-容易踩的坑)
- [这个项目能学到什么](#这个项目能学到什么)
- [系统整体是怎么跑的](#系统整体是怎么跑的)
- [Agent 这块是怎么设计的](#agent-这块是怎么设计的)
- [前置编排器到底做了什么](#前置编排器到底做了什么)
- [RAG 检索链路到底有多细](#rag-检索链路到底有多细)
- [文档从上传到可检索经历了什么](#文档从上传到可检索经历了什么)
- [会话记忆是怎么管理的](#会话记忆是怎么管理的)
- [MCP 工具协议与 Skills 能力扩展](#mcp-工具协议与-skills-能力扩展)
- [项目工程质量](#项目工程质量)
- [和 Demo 项目有什么区别](#和-demo-项目有什么区别)
- [适合什么人](#适合什么人)
- [用到的技术一览](#用到的技术一览)
- [页面长什么样](#页面长什么样)
...
Spring AI / LangChain4j 是好工具，但直接拿来套壳不等于企业级。企业场景下要面对的是：大规模文档的增量更新、高并发下的检索性能、模型调用的成本控制和容错、请求风控、可观测性……这些问题框架的 QuickStart 不会告诉你，但面试官和实际业务一定会考你。
...
- **双通道混合检索——向量检索（PGVector）和关键词检索（Elasticsearch）并行执行，RRF 融合排序，可选外部 Rerank 精排，召回率和精准度兼顾**
...
所有这些步骤完成后，编排器会产出一个 `ConversationExecutionPlan`，里面包含执行模式（CLARIFY / RAG_CHAT / REACT_AGENT）、改写后的问题、拆分后的子问题列表、知识域范围等，交给对应的执行器去执行。
...
- 不是只用向量检索。用户问一个订单号、一个配置项名，向量检索很可能找不到。所以用了**双通道并行**：向量检索走 PGVector，关键词检索走 Elasticsearch
- 两路结果分数量纲完全不同，不能直接比大小。用 **RRF（Reciprocal Rank Fusion）** 按排名倒数法融合
- 向量通道设了最低相似度 0.45，关键词通道用相对阈值 0.35，低于阈值的弱命中直接过滤掉
...
- 如果证据太多，有预算控制：单个子问题最大 2200 字符，全部证据总预算 5200 字符，单个父块最大 2200 字符。防止把模型上下文窗口吃满
